...
% Recall the de Broglie wavelength $\lambda_\text{dB} = h / p$.
% Say, for sake of calculations, that $p^2 / 2m = k_BT$, so at $T_0$
% \[ \lambda_\text{dB} = \frac{h}{\sqrt{2mk_BT_0}} = \frac{h}{\sqrt{2mk_B \frac{3\hbar^2 \pi^2}{2mk_BV^{2 / 3}}}} = \frac{h}{\hbar \pi \sqrt{3}}V^{1 / 3} = \frac{2}{\sqrt{3}} L. \]
% So the de Broglie wavelength of our bosons is about the size of the box.
% This condition is much too restrictive---each boson doesn't need to ``fill'' the entire box to start interacting.
% Perhaps they should, all together, fill the box!
% So our condition becomes
% \begin{align*}
%     V &= N \lambda_\text{dB}^3 \\
%     \lambda_\text{dB} &= \left( \frac{V}{N} \right)^{1 / 3} = \frac{h}{\sqrt{2mk_BT_c}} \\
%     T_c &= \frac{h^2}{2mk_B} \left( \frac{N}{V} \right)^{2 / 3}
% \end{align*}
% This has the same ``shape'' as the $T_0$ we derived earlier, just with an $N^{2 / 3}$ in the numerator.
% Thus for $N \gg 1$ we see $T_c \gg T_0$.
